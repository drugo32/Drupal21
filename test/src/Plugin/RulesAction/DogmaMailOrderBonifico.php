...
namespace Drupal\dogmautil\Plugin\RulesAction;
 ...
 *
 * @RulesAction(
 *   id = "rules_dogma_email_bonifico",
 *   label = @Translation("Send email bonifico"),
 ...
 *       description = @Translation("User pay oredr")
 *     ),
 *     "order" = @ContextDefinition("any",
 *       label = @Translation("Commerce order"),
 *       description = @Translation("Order pay")
 *     ),
 ...
 */
class DogmaMailOrderBonifico extends RulesActionBase implements ContainerFactoryPluginInterface {
   ...
   */
  protected function doExecute(array $to, $subject, $user, $order, $reply = NULL, LanguageInterface $language = NULL) {

      $args = array(
        '@nome' => $user->getUsername(),
        '@link_order' => render($order->toLink(t("tuo ordine"), 'canonical')->toRenderable()),
        '@price' => $order->getTotalPrice()->__toString(),
      );
      $message = t("Ciao @nome.
      In seguito al tuo ordine @link_order su dogmaorganics.com effettuato tramite bonifico bancario
      ti inviamo le istruzioni per effettuare il pagamento di @price
      [ISTRUZIONI]
      Cordiali saluti.
      www.dogmaOrganics.com",$args);
      ...
      $recipients = implode(', ', $to) . ', ' . $order->getEmail();
